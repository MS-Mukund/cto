...
plt.plot(num_targs, final_vec0[0], linestyle='solid', marker='o', color='b', label='kstep controlled')
plt.plot(num_targs, final_vec0[1], linestyle='dashed', marker='D', color='g', label='explexpl')
plt.plot(num_targs, final_vec0[2], linestyle='dotted', marker='^', color='black', label='DQN Target')
plt.plot(num_targs, final_vec0[3], linestyle='dashdot', marker='s', color='r', label='DQN no target')

plt.legend()
plt.title('DQN')    # change title
plt.xlabel('Number of targets')
plt.ylabel('Average percentage')

# plt.show()
plt.savefig('DQN_comparison.png') # change filename
